...
| `get_*` | A data object (tibble, list, vector) | No — ends the pipe | `get_mod_table()`, `get_mod_dbinfo()` |
| `peek_*` | `invisible(mod_db)` | Yes — prints and stores in `last_result` | `peek_mod_table()` |

The key rule: **`get_*` functions return data, not `mod_db`.** They are exit points — use them to pull data out of the database at the end of a workflow, not as intermediate pipe steps.
