...
you can convert them into compressed `.ch3` archives using `make_ch3_archive()`. 
This will dramatically reduce file size and allow use of the ModSeqR package.

`make_ch3_archive()` reads a tab-delimited calls file, selects and transforms the 
...
After the `.ch3` files are written, point `make_ch3_db()` at the directory containing them (or pass their paths directly), and the package will take care of the rest.
...
* You can run make_ch3_archive() for each sample TSV, all writing into the same out_path; make_ch3_db() can then ingest the whole directory in one call.
...
Calling `calc_ch3_diff()` again on the same type of data will overwrite the existing `mod_diff_{type}` table (e.g. `mod_diff_windows` will be rewritten if you run it again on window data).
