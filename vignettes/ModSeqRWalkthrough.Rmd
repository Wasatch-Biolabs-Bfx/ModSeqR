...
* Apply `min_coverage` filtering to drop low-quality windows before testing
...
When working with window-level data, some windows may have poor breadth of 
coverage — reads may only cover a small portion of the window in certain 
samples. The `min_coverage` parameter filters these out before testing.
...
Filtering improves result quality and reduces the multiple testing burden, 
since fewer windows are tested.
