...
Most ModSeqR functions return an object of class `mod_db` — a lightweight list that holds five
pieces of state:
...
| `config` | Resource settings: `memory_limit`, `temp_dir`, `threads` (auto-detected if `NULL`) |
| `.conn_env` | Environment holding the live DBI connection; managed automatically |

Typing a `mod_db` variable — or calling `print()` on it — shows the key slots:
...
`make_mod_db()` closes its build connection and returns a *disconnected* object. The first
pipeline function call opens a connection automatically (lazy open). To connect to a database
that already exists on disk — for example in a new R session — use `connect_mod_db()`:

```{r eval=FALSE}
# Connect to an existing database with auto-detected resources
mod_db <- connect_mod_db("my_analysis.mod.db")

# Optionally specify resource limits
mod_db <- connect_mod_db("my_analysis.mod.db",
                          memory_limit = "32GB",
                          threads      = 8,
                          temp_dir     = "/scratch/duckdb_tmp")
```

Only one connection per database file is allowed at a time. Attempting a second
`connect_mod_db()` to the same file while a connection is live raises a clear error:
...
The connection closes automatically when the `mod_db` object is garbage-collected. You can also
close it explicitly:
...
ModSeqR uses verb prefixes to signal what a function returns:

| Prefix | Returns | Pipe-compatible? | Examples |
|---|---|---|---|
| `make_*`, `summarize_*`, `calc_*`, `plot_*`, `filter_*`, `collapse_*`, `trim_*`, `classify_*`, `run_*`, `export_*`, `rename_*` | `invisible(mod_db)` | Yes | `summarize_mod_windows()`, `calc_mod_diff()` |
| `get_*`, `peek_*` | A data object (tibble, list, vector) | No — ends the pipe | `get_mod_table()`, `peek_mod_table()` |

The key rule: **`get_*` and `peek_*` functions return data, not `mod_db`.** They are exit points — use them to pull data out of the database at the end of a workflow, not as intermediate pipe steps.

```{r eval=FALSE}
# Pipeline steps return mod_db and can be chained
mod_db <- make_mod_db(ch3_files, "my_db") |>
  summarize_mod_windows()                 |>
  calc_mod_diff(cases = c("A","B"), controls = c("C","D"))

# get_* functions exit the pipe and return data
result_df <- get_mod_table(mod_db, mod_db$current_table)
tables    <- get_mod_tablelist(mod_db)   # character vector
info      <- get_mod_dbinfo(mod_db)      # named list
cols      <- get_mod_cols(mod_db, "mod_windows")  # character vector
```

## Capturing in-memory results with `get_mod_result()`

Some pipeline functions compute a result that is printed to the console but not otherwise accessible.
...
You may optionally pass a named character vector to assign explicit `sample_name`s; otherwise, `sample_name` is derived from the file’s basename by removing a trailing -<digits> (if present) and the `.ch3` extension (e.g., `Astrocytes-12.ch3` → `Astrocytes`, `M1.combined.ch3` → `M1.combined`). Additional filters include `chrom` (vector of chromosomes to keep), `min_read_length` (default 50), `min_call_prob` (default 0.9), `min_base_qual` (default 10), and an optional `flag` filter. The function configures DuckDB resource limits at connection time (memory, threads, temp directory for spill-to-disk), drops any existing tables, and creates a `calls` table with: `sample_name`, `chrom`, `start`, `end`, `read_position`, `call_code`, `read_length`, `call_prob`, `base_qual`, and `flag`.

The function returns a *disconnected* `mod_db` object. Pass it directly to any pipeline function — the connection opens automatically on the first use — or call `connect_mod_db()` in a new session to reconnect to the same file.

```{r eval=FALSE}
# Build from a directory of CH3 files; auto-derived sample names
mod_db <- make_mod_db(
  ch3_files = "/path/to/ch3_dir",
  db_name   = "my_db"
)
...
# Explicit sample names (override the auto-derived fallback)
mod_db <- make_mod_db(
  ch3_files = c(
...
# Reconnect to the same database in a later R session
